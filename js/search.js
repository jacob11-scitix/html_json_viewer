...
// - Substring, case-insensitive across keys and primitive values.
// - Hides non-matching subtrees but keeps ancestor chain visible (auto-expanded).
// - Wraps matched substrings in <mark class="match">.
// - Empty query restores original DOM state.
...
function matchesRow(row, query) {
  // Only consider key and value text (not chip / actions / caret).
...
export function applySearch(container, query) {
  clearFilter(container);
  const q = query.trim();
  if (!q) return;

  // First pass — find direct hits.
  ...
  // Build the visible set: hits + all their ancestors + all their descendants.
  ...
  // Highlight matches inside visible rows only.
  for (const hit of hits) {
