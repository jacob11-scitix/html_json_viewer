...
// Exposes window.JV.pagination.renderRecordsView(records, opts).

(function (JV) {
  const { renderTree } = JV.tree;
  ...
  function renderRecordsView(records, opts) {
    opts = opts || {};
    const root = document.createElement('div');
    root.className = 'records-view';

    const state = { pageSize: 20, page: 1 };
    ...
    next.addEventListener('click', () => { if (state.page < pagesTotal()) { state.page++; render(); } });
